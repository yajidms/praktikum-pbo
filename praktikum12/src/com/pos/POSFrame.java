...
public class POSFrame extends JFrame {
  ...
  private void addToCart() {
    ...
    if (row < 0 || qty <= 0) {
      JOptionPane.showMessageDialog(this, "Pilih produk dan qty \u2265 1.", ERROR_TITLE, JOptionPane.ERROR_MESSAGE);
      return;
    }
  ...
  private void editSelectedQty() {
    ...
    try {
      ...
      JOptionPane.showMessageDialog(this, "Qty harus bilangan bulat > 0.", ERROR_TITLE, JOptionPane.ERROR_MESSAGE);
    }
  }
  ...
  private String generateReceipt() {
    ...
    sb.append(String.format("===== POIN OFF-SALES =====%n"));
    sb.append(String.format("Toko: Demo Toko%n"));
    sb.append(String.format("==========================%n"));
    sb.append(String.format("%-5s %-20s %3s %12s%n", "ID", "Nama", "Qty", "Subtotal"));
    ...
    for (int i = 0; i < cartModel.getRowCount(); i++) {
      ...
      total += sub;
      sb.append(String.format("%-5s %-20s %3d %12s%n", id, nama, qty, idr.format(sub)));
    }

    sb.append(String.format("--------------------------%n"));
    sb.append(String.format("TOTAL: %s%n", idr.format(total)));
    sb.append(String.format("POINTS DIDAPAT: %d (1 point per Rp 1000)%n", (int)(total / 1000.0)));
    sb.append(String.format("==========================%n"));
    sb.append(String.format("Terima kasih! Silakan kunjungi kembali.%n"));
    return sb.toString();
  }
